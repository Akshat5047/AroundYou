...
climate_meta = joblib.load(metadata_path)
...
def _load_climate_history():
    ...
    # Rolling 7-day rainfall percentage
    ...
    district_baseline = district_daily.xs(
        last_known_date,
        level="Date"
    )
...
climate_diffed, climate_district_baseline = (
    _load_climate_history()
)
...
def predict_climate(data: dict):
    ...
    if district not in climate_district_baseline.index:
        return {
            "error":
    ...
    current_seq = torch.tensor(
        climate_diffed.values[-seq_len:],
        dtype=torch.float32
    ).unsqueeze(0)

    baseline = (
        climate_district_baseline.loc[district]
    )
